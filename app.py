...
def batch_detect(face_detector, imgs, device):
    # Ensure imgs is a numpy array
    if not isinstance(imgs, np.ndarray):
        imgs = np.array(imgs)

    # Ensure the dtype of the numpy array is float for the subtraction operation
    imgs = imgs.astype(np.float32)
    
    # Separate the RGB channels and the alpha channel
    if imgs.shape[-1] == 4:
        rgb_imgs = imgs[..., :3]
        alpha_channel = imgs[..., 3]
    else:
        rgb_imgs = imgs
        alpha_channel = None

    # Perform the subtraction operation on the RGB channels only
    rgb_imgs = rgb_imgs - np.array([104, 117, 123])

    # Combine back the RGB channels and the alpha channel if it exists
    if alpha_channel is not None:
        imgs = np.dstack((rgb_imgs, alpha_channel))
    else:
        imgs = rgb_imgs
        ...
        if uploaded_file is not None and uploaded_img_file is not None:
            ...
            if st.button("립싱크 영상 생성하기"):
                ...
                with st.spinner("영상 파일 생성 중..."):
                    result_filenames = main(img_save_path)
