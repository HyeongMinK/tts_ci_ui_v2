...
def load_model(path):
    ...
    with torch.no_grad():
        conv1_weight = model.face_encoder_blocks[0][0].conv_block[0].weight
        ...
        for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
            ...
            if i == 0:
                ...
                out = cv2.VideoWriter('temp/result.avi',
                                      cv2.VideoWriter_fourcc(*'DIVX'), fps, (frame_w, frame_h), isColor=True)  # RGBA 형식 지정
...
).format(audio_file_path, 'temp/result.avi', result_filename)
